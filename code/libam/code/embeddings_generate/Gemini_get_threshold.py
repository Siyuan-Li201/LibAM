...
from Gemini_utils import get_f_name, get_f_dict, read_graph, partition_data
...
parser.add_argument('--data_path', type=str,
                    default='/data/wangyongpan/paper/reuse_detection/datasets/dataset-elf-exec/results/', help='path for all data')


def draw_dot_graph(scores, truths, save_path="", node_num=200):
...
def contra_loss_show(net, dataLoader, DEVICE, save_path=""):
    ...
    draw_dot_graph((1+cos)/2, truth, save_path)
    fpr, tpr, thres = roc_curve(truth, (1+cos)/2)
    model_auc = auc(fpr, tpr)
    #return loss_val, model_auc, tpr
    return model_auc, fpr, tpr, thres, len(truth)
...
if __name__ == '__main__':
    ...
    auc1, fpr1, tpr1, thres1, num_pairs = contra_loss_show(net, testLoader, DEVICE, save_path="train_dot.jpg")
    print("testing auc = {0} @ {1}, pairs = {2}".format(auc1, datetime.now(), num_pairs))

    print(max((1 - fpr1 + tpr1) / 2))
    index = np.argmax(1 - fpr1 + tpr1)
    print("index:", index)
    print("recall non-homology ", 1 - fpr1[index])
    print("recall homology:", tpr1[index])
    print(thres1[index])

    auc2, fpr2, tpr2, thres2, num_pairs = contra_loss_show(net, validLoader, DEVICE, save_path="valid_dot.jpg")
    print("valid auc = {0} @ {1}, pairs = {2}".format(auc2, datetime.now(), num_pairs))

    print(max((1 - fpr2 + tpr2) / 2))
    index = np.argmax(1 - fpr2 + tpr2)
    print("index:", index)
    print("recall non-homology ", 1 - fpr2[index])
    print("recall homology:", tpr2[index])
    print(thres2[index])

    auc3, fpr3, tpr3, thres3, num_pairs = contra_loss_show(net, trainLoader, DEVICE, save_path="test_dot.jpg")
    print("train auc = {0} @ {1}, pairs = {2}".format(auc3, datetime.now(), num_pairs))

    print(max((1 - fpr3 + tpr3) / 2))
    index = np.argmax(1 - fpr3 + tpr3)
    print("index:", index)
    print("recall non-homology ", 1 - fpr3[index])
    print("recall homology:", tpr3[index])
    print(thres3[index])
